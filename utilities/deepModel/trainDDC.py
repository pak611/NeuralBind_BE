...
def get_parser():
    ...
    parser.add_argument("--train_csv", help="train file",
                        #default='C:\\Users\\patri\\Dropbox\\Ph.D\\OrganoNet\\NeuralBind\\nbBackend\\fingerprin\\IFP_AAIFP_dScorePP.csv')
                        default='IFP_AAIFP_dScorePP.csv')
    parser.add_argument("--trainDic", help="pickle file for training",
                        default='referAtomsList')
...
def main(args):
    saveDir = Path(args.save).parent
    saveDir.mkdir(parents = True, exist_ok= True)
    
    if args.load_pkl <= 0:
        print('Processing csv')
        fullDf = pd.DataFrame([])
        os.chdir(f'{os.getcwd()}\\nbBackend\\fingerprint\\')
    ...
    IFPmodel = deepModel.diffusionModel(
        x=bitList, y=molList, dataset_info=dataset_info, scaling=True)
    IFPmodel.fit(epochs=50, lr=0.001, mini_epochs=1, model_name='fullBits', gpus=1, patience=1, checkpoint_dir=args.save,
