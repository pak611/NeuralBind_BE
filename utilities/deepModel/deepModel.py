...
#from tensorflow.python.compat.v1.keras.layers import (
#    CuDNNLSTM as LSTM,
...
import shutil
import os
#from load_picklerick import load
...
class diffusionModel:
    def __init__(self, **kwargs):
        ...
        if self.mode == "train":
            ...
            self.__batch_size = kwargs.get("batch_size", 9)
    ...
    def __load(self, model_name):
        ...
        #temp_dir = 'C:\\Users\\patri\\Dropbox\\Ph.D\\Research\\Doctoral_Research\\Cheminformatics\\IFP-RNN\\ddc\\ddc_pub\\saved_model'
        #temp_dir = 'C:\\Users\\patri\\Desktop'
        ...
        #from ddc_pub import ddc
        # Load metadata
        metadata = pickle.load(open(temp_dir + "/metadata.pickle", "rb"))

        

        #dirpath = 'C:\\Users\\patri\\Dropbox\\Ph.D\\Research\\Doctoral_Research\\Cheminformatics\\IFP-RNN\\ddc\\ddc_pub\\saved_model'

        #metadata = load(self.model_name)
        ...
        try:
            self.__mol_to_latent_model = load_model(
                temp_dir + "/mol_to_latent_model.h5"
            )
        except:
        ...
        self.__latent_to_states_model = load_model(
            temp_dir + "/latent_to_states_model.h5"
        )
        self.__batch_model = load_model(temp_dir + "/batch_model.h5")
    ...
    def fit(
        ...
        save_period=5,
        #checkpoint_dir="/",
        checkpoint_dir='C:\\Users\\patri\\Dropbox\\Ph.D\\Research\\Doctoral_Research\\Cheminformatics\\IFP-RNN\\ddc\\ddc_pub\\saved_model',
        lr_decay=False,
        ...
        if clipvalue > 0:
            ...
            opt = tf.keras.optimizers.Adam(learning_rate=self.lr, clipvalue=self.clipvalue)
        ...
        checkpoint_file = (
            checkpoint_dir + 'saved_model_file.txt'
        ...
        if gpus > 1:
            ...
            parallel_model = mirrored_strategy(self.model, gpus=gpus)
            ...
            self.model.fit_generator(    # _generator
                self.train_gen,
                steps_per_epoch=int(round(self.steps_per_epoch / mini_epochs)),
                epochs=int(round(mini_epochs * self.epochs)),
                validation_data=self.valid_gen,
                validation_steps=int(round(self.validation_steps / mini_epochs)),
                callbacks=callbacks,
                max_queue_size=10,
                workers=workers,
                use_multiprocessing=use_multiprocessing,
    ...
    def predict(self, latent, temp=1, rng_seed=None):
        ...
        smilesvec_look = self.smilesvec1
